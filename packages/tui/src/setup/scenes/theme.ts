...
class ThemeSceneController implements SetupSceneController {
	...
	async #commit(value: string): Promise<void> {
		if (value === "auto") {
			...
			// saving a hard-coded dark theme here used to overwrite the configured one
			// on a bare Enter. Only pin the light mapping; the dark mapping keeps the
			// user's saved choice and auto-detection re-evaluates against it.
