import { containsFullsend, highlightFullsend } from "./fullsend";
 ...
 * "workflowz", "fullsend") that appears as standalone prose, skipping any
 * occurrence inside a code block, inline code span, or XML/HTML section. Each highlighter paints its
...
export function hasMagicKeyword(text: string): boolean {
	if (
		...
		!text.includes("fullsend")
	) {
		return false;
	}
	return containsUltrathink(text) || containsOrchestrate(text) || containsWorkflow(text) || containsFullsend(text);
}
