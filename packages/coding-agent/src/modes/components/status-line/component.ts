...
interface CachedStatusLine {
	...
	/** Segments the width shrink dropped, in render order; wrapped into rows by the callers that show them. */
	overflowParts: readonly string[];
	width: number;
...
export class StatusLineComponent implements Component {
	...
	#buildStatusLine(
		...
		const result = {
			...
			overflowParts: rendered.overflowParts,
	...
	#renderStatusLine(
		...
		nowMs: number,
	): { content: string; overflowParts: readonly string[] } {
		const effectiveSettings = this.#resolveSettings();
		this.#syncPricingTimer();
		const placeholders = options?.placeholders === true;
	...
	#renderSplitBottomLines(width: number, previewTitle?: string): string[] {
		const left = this.#renderStatusLines(width, "plain-left", previewTitle);
		...
		const right = this.#renderStatusLines(Math.max(1, width - CHIP_RULE_RESERVE), "plain-right", previewTitle);
	...
	#renderStatusLines(
		...
		placeholders = false,
	): { main: string; overflow: string[]; overflowParts: readonly string[] } {
		const statusLine = this.#buildStatusLine(width, layout, previewTitle, { placeholders });
		const rows = this.#wrapOverflowRows("", statusLine.overflowParts, overflowWidth ?? width);
		if (!this.#focusedAgentId)
			return { main: statusLine.dimmedContent, overflow: rows, overflowParts: statusLine.overflowParts };
		const dim = (content: string): string =>
			content ? `\x1b[2m${content.replaceAll("\x1b[0m", "\x1b[0m\x1b[2m")}\x1b[22m` : "";
		return { main: statusLine.dimmedContent, overflow: rows.map(dim), overflowParts: statusLine.overflowParts };
	}
	...
	getStandaloneTopBorder(width: number, previewTitle?: string): { content: string; width: number; revision: number } {
		...
		const statusLine = this.#buildStatusLine(Math.max(1, width - CHIP_RULE_RESERVE), "plain-right", previewTitle);
		return {
			content: statusLine.dimmedContent,
			width: statusLine.width,
			revision: this.#renderRevision,
