...
export class OAuthSelectorComponent extends OverlayPanel {
	...
	#loadProviders(): void {
		...
		if (this.#mode === "logout") {
			// Logout stays unfiltered by the allow-list: a now-disabled
			...
			// Hide a login entry unless the model provider it authenticates is
			// enabled, so alias logins (e.g.
			// `openai-codex-device` ⇒ `openai-codex`) disappear alongside the
			// model provider they authenticate.
			this.#allProviders = providers.filter(provider =>
				isProviderEnabled(provider.storeCredentialsAs ?? provider.id),
			);
		}
