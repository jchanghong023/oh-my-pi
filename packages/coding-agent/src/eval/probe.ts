...
export async function runBoundedProbe(
	command: string[],
	{ cwd, env, signal, timeoutMs, maxTimeoutMs, timeoutCeilingMs }: BoundedProbeSpawnOptions,
): Promise<BoundedProbeResult> {
	if (signal?.aborted) {
		return { exitCode: null, timedOut: false, aborted: true };
	}
	const defaultCeiling = Math.max(timeoutCeilingMs ?? 0, DEFAULT_PROBE_TIMEOUT_MS);
	const ceiling = maxTimeoutMs && maxTimeoutMs > 0 ? maxTimeoutMs : defaultCeiling;
