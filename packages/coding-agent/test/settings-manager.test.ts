...
describe("Settings", () => {
	...
	describe("config file failure safety", () => {
		...
		// Windows cannot create file symlinks without privilege, so this chain fixture cannot be built.
		it.skipIf(process.platform === "win32")(
			"follows an intermediate directory symlink inside a relative target before applying ..",
			async () => {
		...
		// Windows cannot create file symlinks without privilege, so this chain fixture cannot be built.
		it.skipIf(process.platform === "win32")(
			"resolves an absolute target's intermediate directory symlink before applying ..",
			async () => {
		...
		// Windows cannot create file symlinks without privilege, so this chain fixture cannot be built.
		it.skipIf(process.platform === "win32")(
			"does not mislocate when a dangling component precedes further names then ..",
			async () => {
		...
		// Windows cannot create file symlinks without privilege, so this chain fixture cannot be built.
		it.skipIf(process.platform === "win32")(
			"rejects with ENOTDIR when a `..` target's component is created as a regular file mid-walk",
			async () => {
		...
		// Windows cannot create file symlinks without privilege, so this chain fixture cannot be built.
		it.skipIf(process.platform === "win32")(
			"rejects with ENOTDIR when a `..` target's directory is removed before the validation stat",
			async () => {
				// config.yml -> racetarget/../victim.yml, where `racetarget` does not
	...
	describe("get()", () => {
		it("resolves overrides, schema defaults, and falsey values", () => {
			...
			expect(cfgStatusLineCompactThinkingLevel.get(isolated)).toBe(false);
		});
