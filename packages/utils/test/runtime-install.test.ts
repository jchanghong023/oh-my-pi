...
describe("ensureRuntimeInstalled install lock", () => {
	...
	// bun 1.4 on Windows fails `file:` installs with an EPERM cache-copy error
	// (exit 1 even though the files land), so this install flow cannot succeed there.
	test.skipIf(process.platform === "win32")(
		"a stale legacy .lock directory does not block install and is cleared",
		async () => {
	...
	// (exit 1 even though the files land), so this install flow cannot succeed there.
	test.skipIf(process.platform === "win32")(
		"a live legacy .lock owner is waited out, never reclaimed on retry exhaustion",
		async () => {
			const { spec, probe } = await makeFileDependency();
			const runtimeDir = await makeRuntimeDir();
	...
	// (exit 1 even though the files land), so this install flow cannot succeed there.
	test.skipIf(process.platform === "win32")(
		"reserves the legacy namespace before the new install starts",
		async () => {
			const { spec, probe } = await makeFileDependency();
			const runtimeDir = await makeRuntimeDir();
	...
	// (exit 1 even though the files land), so this install flow cannot succeed there.
	test.skipIf(process.platform === "win32")(
		"a crashed installer does not wedge a later install",
		async () => {
			const { spec, probe } = await makeFileDependency();
			const runtimeDir = await makeRuntimeDir();
