...
bash 工具具有 `exec` 审批层级。`bash.patterns` 规则可以显式地 `allow`、`deny` 或 `prompt`。默认情况下，allow 规则必须匹配整条命令，且不能批准 shell 控制语法。可选开启的 `bash.allowCompoundCommands: true` 额外仅识别由未加引号的 `&&` 分隔、由两条或多条字面命令组成的扁平链。它会针对每个原始片段独立解析这组有序规则，该片段以第一条匹配的规则为准。

限制会在整条链上以保守方式合并：任何显式的片段级或整链 `deny` 优先，否则任何显式的 `prompt` 优先。匹配完整链但不匹配任何单个片段的 `deny` 或 `prompt` 模式仍保持为整链限制，而靠后出现的宽泛限制不会覆盖某个片段上更早的匹配。这些限制在既有的原始与规范化关键命令检查之前解析。

整链限制在扫描时以 deny 为优先，即使匹配的 prompt 出现得更早。集中式的 shell 正向分类器仅对已知采用 POSIX 引用约定的 shell（`sh`、`bash`、`dash`、`ash`、`ksh` 和 `zsh`，包括 `.exe` 名称）允许复合识别。Cmd、PowerShell、fish 以及未知 shell 保留既有审批行为；仅在执行时接受 `-c` 并不足以证明其具备兼容的引号处理。

在这些检查之后，只有当每个片段都显式解析为 `allow` 时，该命令链才会获得显式的 `write` 层级 allow。只要任一片段未匹配，bash 就保留其独立的 `exec` 层级且不带显式策略，因此通用审批解析器会依次应用 `tools.approval.bash` 和当前激活的模式。因此，未匹配的片段会继承既有策略，而不是一律触发提示。

带引号的字面参数是可接受的，但展开、赋值、其他控制操作符、重定向、通配符展开、换行、格式错误的语法以及改变 shell 状态的内建命令（`cd`、`source`、`eval` 等）不符合条件。这些输入保留既有审批行为。关键破坏性以及远程获取并执行的检查仍会检查完整的原始与规范化输入及其各个片段，因此获得允许的前缀无法掩盖其后关键性的片段。审批不会改写执行：shell 收到的仍是原始命令，保留原生的 `&&` 短路行为。

模式审批并非隔离（containment）。一旦获得批准，进程仍保留该 shell 环境下对文件系统、网络和子进程的访问能力。拦截与审批同样是相互独立的机制：拦截将误用引导至专用工具；审批决定执行是否可以继续。
...
并发调用绝不会共享同一个 `Shell`：原生会话一次只运行一条命令，且 `Shell.abort()` 会终止该会话上所有正在进行的运行。`executeBash()` 在 `shellSessionsInUse` 中跟踪正在进行中的 key；在 key 处于忙状态时，重叠的调用会跳过缓存并通过一次性 `executeShell()` 运行（与隔离会话相同的隔离级别）。只有拥有该调用的那一方会在其 `finally` 中释放 in-use 标志或删除缓存的会话。
