...
顾问永远不是对等体。`advisor` 类型的注册表引用被排除在所有面向代理的表面之外 —— `hub` 对等体名单和广播目标、子代理对等体提示，以及 `history://` 索引/查找/补全 —— 并且不能向它发送消息（`hub` 发送和 collab 聊天会拒绝它），也不能[从 Agent Hub 恢复或终止](./agent-hub.md#persisted-agents-and-advisors)或从 collab 中恢复或终止。无论被授予了什么工具，它都不能作为对等体寻址。
