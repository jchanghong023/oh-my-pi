...
`<encoded-cwd>` 是对路径编码后的 canonical cwd(在 home 下为 `-<relative>`，在 temp 根目录下为 `-tmp-<relative>`，其他情况下为 `--<encoded-absolute>--`；参见 [session.md](session.md#磁盘布局))。已回滚的 17.2.5-17.2.8 哈希分桶方案会尽力迁移。`SessionManager.list(cwd, sessionDir?)` 仅读取已解析分桶，除非显式提供 `sessionDir`。
...
- 选择器还会在 all-projects 范围内显示修改时间、文件大小、活动会话上的 `current` 标记、生命周期状态(除 `unknown` 外)、fork 标记以及 cwd

## `--continue` 解析与终端 breadcrumb 偏好
...
2. 若为空，仅探测 `SessionManager.listAll()` 以区分全局空状态并预加载 Tab 作用域；选择器本身绝不会自动切换到 all-projects 作用域(issue #3099)
3. 若两个列表都为空，打印 `No sessions found` 并退出
4. 打开全屏 TUI 选择器(`selectSession`)
5. 若取消，打印 `No session selected` 并退出
6. 选中后，先 `SessionManager.open(selected.path)`，再将进程/项目作用域状态切换到该会话的 cwd(`switchToResumedProject`:`setProjectDir`、plugin-cache 重置、settings 重载)，并重新解析作用域内的模型
...
2. 通过 `ctx.ui.showOverlay` 将 `SessionSelectorComponent` 呈现为全屏 alternate-screen overlay(锚定在左上角并占满全屏；其下的 transcript 不受影响)，并接入懒加载的 all-project 加载(`loadAllSessions`)、`history.db` prompt 匹配器、删除功能、pinned-session 标记以及 current-session 标记
3. 回调：
   - select -> 锁定选择器输入并调用 `handleResumeSession(sessionPath)`；成功时隐藏 overlay 并恢复编辑器焦点，可恢复的切换前失败则解锁选择器并保持其打开
   - cancel -> 隐藏 overlay、恢复编辑器焦点并重渲染
   - exit -> 隐藏 overlay，然后 `ctx.shutdown()`
...
- 当提供 `currentSessionPath` 时，当前活动会话会在其元数据行上标记为 `current`，并在打开时以及 Tab 作用域切换后获得焦点
...
5. `sessionManager.setSessionFile(sessionPath)`：更新 breadcrumb，加载/迁移/blob-resolve/index 条目，并在 cwd 策略允许时采用已存在的 recorded cwd
...
13. 对不同会话恢复 advisor cost，完成 bash 转换，通知 session-change 回调，成功时返回 `true`

`switchSession()` 在 before-switch 钩子取消或 cwd 策略拒绝该转换时返回 `false`。没有 cwd 变更回调的跨项目切换会被拒绝，而不是静默采用目标 cwd；回调被拒绝同样构成取消。

快照之后的任何失败都会恢复之前的 manager 和 runtime state，重新连接/reconcile 它，将 bash 转换标记为失败，然后重新抛出。

## 交互式切换后的 UI 状态重建
`SelectorController.handleResumeSession` 会先调用 `switchSession`。如果它返回 `false`，选择器会在应用任何新会话 UI 更新之前停止，并保持既有会话/UI 不变。切换成功后，它会：
...
## 启动时 resume 与会话内切换
### 启动时 resume（`--continue`、`--resume`、直接打开）
...
- 核心钩子或 cwd 策略取消 -> `switchSession()` 返回 `false`；交互式选择器会在其 UI 刷新/状态路径之前停止，保留旧会话与 UI。无回调的跨项目切换会被拒绝，而不是静默采用目标 cwd。
