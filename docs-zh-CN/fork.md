...
* **版本**：`v18.2.6`
* **Upstream commit**：`936f9762d2682a7b19090b090c2047be0d42e3f4`
* **同步日期**：2026-09-21
...
* `find` 是只读工具（`approval = "read"`），已加入 `task/read-only-policy.ts` 的只读工具集合：上游把 `find` 加进 `scout` 等内置代理的 `tools:` 后没有同步该集合，导致 scout 被误判为可写代理（Agent Hub 不再显示 `Read-only`，`task` 委派的只读分类与持久化 `readOnly` 标记随之失真）；上游补齐后按同步流程撤除。
