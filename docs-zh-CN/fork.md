...
- **主代理切换**：TUI 在自动补全未接管且主会话空闲时，以 `Tab` 在同一会话的默认主代理与讨论主代理间切换；讨论主代理仅保留只读调查工具，禁止执行命令、写入、todo 和子代理委派；工具目录刷新保留动态挂载的 `xd://` 设备。
...
- **安装/更新体验**：`install.sh` 与 `install.ps1` 下载前比较目标路径已安装版本与 release tag，相同时跳过下载（兼容 Windows PowerShell 5.1 与 PowerShell 7）；替换前先终止目标路径正在运行的旧 omp（Linux 按 `/proc/*/exe` 解析，Windows 按进程 Path 匹配）；下载到临时文件后原子替换（Linux 用 `mv`，Windows 用 `Move-Item`），避免 ETXTBSY/curl 23 与 exe 文件锁；Windows `curl.exe` 下载失败时自动回退 `Invoke-WebRequest`；安装与 `omp update` 下载显示进度条（百分比/速度/ETA），失败时输出 URL、HTTP 状态与服务器错误正文。
