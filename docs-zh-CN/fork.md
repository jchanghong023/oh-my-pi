...
* **版本**：`v18.2.0`
* **Upstream commit**：`c5a8e0e09296290c2fd3ef053391081acc691ded`
* **同步日期**：2026-09-15
...
* OpenRouter 的同名 `deepseek/deepseek-v4.1-flash` 由上游内置条目独立提供，不属于本继承面；其图片输入由上游 DeepSeek 类规则覆盖（`classes/deepseek.kdl` 的 `models "*v4.1-flash*"` 关闭图片剥离，取代原先 `openrouter.kdl` 的 provider 级条目）。
* V4.1 Flash 原生支持图片输入，而被继承的 `deepseek-v4-flash` 条目为纯文本，其 id 也不含 `vision` / `ocr` 字样、会命中 DeepSeek 类规则默认的图片剥离。类规则对带版本号的 `*v4.1-flash*` 与裸别名 `deepseek-flash` 均关闭剥离，继承面仍同时携带输入模态（`text` + `image`）与「不剥离图片」的覆盖；图片按原样交给网关，不做本地降级。上游 `opencode-go.kdl` 已自行声明该 provider 的模态、不剥离图片与 `effort` 思考模式（档位来自 DeepSeek 类规则的 flash 家族），此处继承面补充显示名与限额；官方 `deepseek` 与 `opencode-zen` 的模态与剥离项仍来自继承面。思考等级、限额与显示名仍与同类 DeepSeek 模型一致。
...
* 内置目录出现这些 id 的正式条目后自动以条目为准。上游已为官方 `deepseek` 裸别名补齐等级阶梯与 wire 契约，本继承面在该 lane 只剩显示名与图片模态；`opencode-go`、`opencode-zen` 的发现映射仍由本继承面提供显示名、家族与限额（KDL 侧已带分类 family、模态与 `effort` 思考模式）；`commandcode` 完全由上游 KDL 规则覆盖，本继承面不再涉及该 provider。
* `commandcode` 的 `deepseek/deepseek-v4.1-flash` 由上游 KDL 规则提供等级（`low` / `high` / `max`）与图片输入，价格取上游 `cost-patch`；官方 `deepseek` 的 `deepseek-flash` 由上游条目提供推理等级与 wire 契约，本继承面在该处只补显示名与图片输入。
* 分类侧的 `*deepseek-flash` glob（尾锚定，把以 `deepseek-flash` 结尾的裸 id 归入 Flash 家族）已是上游规则，fork 不再保留同名增量；`deepseek-flash-v4` 这类以版本段结尾的 id 不被卷入（其 id 不含 `deepseek-v4` 段，回落为无 family 的通用档位，与 models.json 烘焙的 identity 一致）。
...
* 同步时需重新应用的测试级适配（上游重写对应文件后会丢失，且上游 CI 覆盖不到）：`is_regular_file` 需导入 `pi-builtins` 的测试宿主模块（fork 自有代码，上游不编译该目标）；`pi-shell` 的 jobspec 强杀测试预算放宽到 600 秒，并把 `//crates/pi-shell:pi-shell_test` 的 bazel 目标超时设为 `long`（默认 300 秒上限会先于测试内预算触发；CI 分片 CPU 争用，实测在 5 / 15 / 60 / 180 秒各失败过一次）；mcp stdio pidfile 轮询只接受活进程 pid；`startup-composer-graph` 测试把注册表路径归一化为 `/`（Windows）；`session-code-mode` 的「保留启动工具体数组引用」用例在 fork 下跳过——应用工具一律经 Primary Agent 执行期门禁包装（Main 下捕获的句柄切到 Discuss 后必须被拒绝），数组引用必然重建，与上游新增的引用复用契约互斥。
