...
* `/jchfuncreview`、`/jchfuncreviewfix`、`/jchverify` 的范围参数为空参（未提交改动）、`commit <ref>`、`path <路径>` 或 `repo`（全仓）；非法 verb 在执行前报用法错误。
