...
* **版本**：`v18.3.2`
* **Upstream commit**：`399f22424a869f0f017f0e62846c0e045f6cf162`
* **同步日期**：2026-09-26
...
* `/repo` 是当前项目的交互索引面板，不是 `omp repo` CLI 命令组；显示实际根目录后由用户首次建立，支持状态、完整内容核对（update）、重建、损坏或不兼容索引的重建恢复、删除及进度/取消。删除只清除索引，不改源码。建库、核对、重建在成功提交完整代次前不暴露半成品；失败或取消保留上一份可用代次，删除与在途维护协调，避免旧任务重新发布。
* 以当前 Git worktree 根为索引范围，非 Git 项目以当前工作目录为范围；路径规范化，每个用户 profile 的目录中按根路径分别保存独立 SQLite 数据库，不修改 Markdown `wiki` 的数据库、命令或“删除后重新导入”流程。无索引时不会因查询自动建库；数据库保留文本、相对路径、类别、内容指纹和 Python 定义，查询不必重新扫描源树。
* 面板和只读 `repo` 工具共用索引：`status` 报告根目录、代次、文件及符号数、最近完整核对、已知待更新路径、未核对原因、失败与排除范围，不扫描全仓；`search` 查已入库文本、返回类别与原始行号片段；`symbol` 查 Python 模块、类、函数和方法的名称/限定名称、定义范围与可用签名，不解析引用或调用关系。查询可按相对路径/目录、文件类别筛选，每页默认 20 条、至多 50 条；代次绑定的 cursor 在索引变动后拒绝旧分页。缺索引、正常无命中与查询错误不同，输出受有界候选/片段和字段截断约束。
* 文本查找按 JavaScript `toLowerCase()` 作不区分大小写匹配：优先完整边界字面量、其次原样子串，拆词补充召回（如 `timeout` 命中 `ERR_TIMEOUT`）；短于三字符的查询也可匹配，标点按普通文本而非 SQL 查询语法处理。结果用于定位而非逐处穷尽；已知路径直接 `read`，编辑前必须读取当前文件，找全使用位置仍用当前文件的 `grep`。Python 定义由内置语法解析提取；其他语言、配置与测试只有文本检索。Python 解析失败时保留可用文本、移除该文件旧定义并报告失败；索引不提供依赖图、影响分析、测试覆盖、向量检索或自动摘要。
* 纳入现有文件搜索忽略规则下的文本文件（源码/测试/配置/其他）；隐藏文件可纳入，符号链接、二进制/非 UTF-8、超过 2 MiB 的文件及不可读取项被排除或报告，忽略规则不是访问控制。成功的 omp 文件编辑标记准确路径（重命名含旧/新路径），下次 `search`/`symbol` 前合并处理已知变更；任意 bash/eval 执行和会话切换将未知变更标记为未核对，Git 状态/HEAD 仅作候选线索。`.ignore` 等忽略或 Git 范围规则变化只能在完整核对后恢复覆盖范围结论；外部改动、未跟踪新增或同大小同时间戳的修改需在面板执行完整内容核对以确认，日常查询不会每次全仓重扫。状态的“最近完整核对”不保证此刻仍与外部文件一致；核对失败、读取竞争或取消须保留旧代次及未核对提示。
...
* 运行承载于会话 async job（`/tan` 同款机制）：阶段进度经任务状态呈现、子代理明细在 Agent Hub、取消经后台任务取消传播到所有在跑子代理。最终结果为一条 markdown 消息落入会话（含统一验收标准、方案状态结构化追踪表、参与完整性说明、固定收尾契约；存在实质影响选择的需求理解差异时置顶提示带答案重跑）。对齐或综合子调用失败，或综合正文自行写入推荐标记、推荐被结构化追踪否决的方案时，报告流程未完成，不输出半成品结论；提案/审查/修订子代理失败如实标注参与不完整、其余继续。
