...
* **版本**：`v18.1.18`
* **Upstream commit**：`f5553d3dd1b197ade36c94b18b6890ae8d9e9e52`
* **同步日期**：2026-09-13
...
* 全部由外部命令构成的多命令管线（如 `a | b &`）按真实后台作业跟踪：jobspec 保留每个管线进程，`kill %1`、`wait %1` 作用于整条管线；含别名、内建或函数的管线仍按上游语义作为内部过程处理。实现为 `crates/vendor/brush-core` 的 fork 补丁，上游更新该文件时需重新适配。
