...
- **Markdown 文档索引**：新增外部 Markdown 目录持久索引与内嵌 `dft` schema；`/docs` 管理索引，用户通过 `/doc <question>` 或“用文档子代理…”显式触发 `@smol` 的 `doc-researcher`，自然语言触发仅注入当轮路由指令，`docs` 工具仍不进入主代理及默认代理上下文；默认仅建 FTS，结构化提取需显式选择，`init/reinit` 以隐藏代际全量构建并原子切换。
