...
| Crate           | 路径                                              | 角色与使用者                                                                                                                                              |
| --------------- | ------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pi-natives`    | [`crates/pi-natives`](../crates/pi-natives)       | 顶层 N-API `cdylib`。它对外暴露 JS 可见的 API，并依赖 `pi-ast`、`pi-iso`、`pi-shell`、`pi-vcs`、`pi-voice` 和 `pi-walker`。                    |
...
| `pi-vcs`        | [`crates/pi-vcs`](../crates/pi-vcs)               | 进程内版本控制：基于 gitoxide 的 git（git 二进制仅在需要凭据的网络传输与 reftable 仓库时保留）以及基于 jj-lib 的 Jujutsu；由 `vcs*` 原生绑定使用的统一发现与操作接口。 |

## Vendored workspace crate
