...
共享的 `readSseEvents` 解码器支持 LF、CRLF 和单独的 CR，包括跨 chunk 拆分的分隔符。传输消费方中的 JSON 解析错误会拒绝该请求/监听器。
...
`src/mcp/json-rpc.ts` 提供 `callMCP()` 与 `readMcpJsonRpcResponse()`，用于 Exa 所使用的直接 HTTP MCP 调用，独立于 `MCPClient`/`MCPManager` 使用的 `MCPTransport` 抽象。

- `callMCP()` 保留所发送请求的 ID；`readMcpJsonRpcResponse(response, expectedId, signal?)` 依据响应内容类型解码 JSON 或 SSE。
- SSE 使用共享的 `readSseEvents` 解码器，包括多行 data 以及没有后续空格的 `data:` 字段。
- 只有与请求 ID 匹配的 result/error 信封才会完成该调用。合法的通知、服务端请求以及其他响应 ID 会被跳过；畸形消息或流耗尽仍未出现匹配响应时则失败。
- 调用方取消仍然是中止（abort），而不是缺失响应错误。仅当未提供调用方信号时才应用硬编码的 60 秒超时。
- 返回的共享 `JsonRpcResponse` 其 result 为 `unknown`；消费方自行收窄其负载。
- 这条轻量路径不管理会话、不应答服务端请求，也不恢复流。
