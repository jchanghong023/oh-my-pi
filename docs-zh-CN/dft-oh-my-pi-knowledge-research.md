...
2. **`wiki` 工具是查询面，`omp docs` 与 `/wiki` 是管理面。** 用户一次性导入、查看或删除索引；Agent 通过只读 `wiki` 执行 `status → search → read`，读取数据库保存的完整章节并引用路径与行号。
...
支持的动作：`init | list | status | remove`。`--json` 输出机器可读结果；导入必须提供 `--name`，删除必须显式 `--force`。没有 `--schema`、`--mode` 参数。
...
`wiki` 只查询既有索引，不提供导入或删除操作。推荐调用顺序：

```text
status
  → 选择一个索引并固定 index
  → search(query)
  → read(sectionId)             # 读取完整 Markdown 章节
```

无索引时，Agent 提示用户运行 `omp docs init "<dir>" --name "<name>"`。存在多个索引时，除 `status` 外必须指定准确的 `index`，防止跨语料混查。
...
`search` 默认最多返回 10 条、硬上限 50 条；内部先获取 `max(100, limit × 4)` 条 BM25 候选，再按以下优先级重排：

1. 完整查询片段命中数：连续中文，以及保留符号、检查边界的技术名称；
2. 当前章节标题中的完整查询匹配；
3. 正文、标题路径或文件路径中的完整查询匹配；
4. 当前章节标题的查询片段覆盖数；
5. 原始 BM25 分数，最后用章节 ID 稳定打破平局。

字面比较采用 NFKC、忽略大小写并折叠空白；同时检查存储原文与现有 Markdown 纯文本提取结果，避免行内格式拆开中文词组。只重排，不删除宽松 AND 命中；候选窗口外的结果仍可能遗漏，不保证全库精确匹配永远在前。

结果按**章节**而不是按文件去重，宽泛词可能由同一文件的多个章节占满 Top-K。每条结果仍带 `sectionId`、索引名、相对路径、标题路径、行范围、摘录和原始 BM25 `rank`；返回顺序以重排结果为准，不应仅按 `rank` 重新排序。`read(sectionId)` 返回完整的存储原文。此优化不改变数据库结构，不需要源文档或重建索引。
...
| `op` | 关键输入 | 返回 |
|---|---|---|
| `status` | 可选 `index` | 索引状态、文档/章节计数及已有错误 |
| `search` | `query`；可选 `limit` | 匹配章节的标识、路径、行号、摘录 |
| `read` | `sectionId` | 完整的存储 Markdown 章节 |

`limit` 默认 10、最大 50。不存在 `lookup`、`relations`、`conflicts` 或 `evidenceId` 接口。
...
6. **边界**：字面命中不等于最终答案正确；Agent 仍需 `read` 原始章节并引用证据。语义问题、并发、p95、峰值磁盘、模型端到端质量等需要独立评测。
