...
| 字段 | 类型 | 必填 | 描述 |
|---|---|---:|---|
| `items` | `Array<{ content: string; context?: string }>` | 是 | 一条或多条要存储的记忆。`minItems: 1`。每条记录必须自包含；`context` 是可选的逐条来源信息。 |
...
- Hindsight 心智模型默认值： `hindsight.mentalModelsEnabled = true`、`hindsight.mentalModelAutoSeed = true`、`hindsight.mentalModelMaxRenderChars = 16_000`。渲染出的 `<mental_models>` 块对当前转录是冻结的——后台 reflect 会在下一个转录/会话边界重新加载它时生效，而 `/memory mm reload` 会强制一次显式的会话内刷新。首轮加载等待最多 `MENTAL_MODEL_FIRST_TURN_DEADLINE_MS = 1500`。
