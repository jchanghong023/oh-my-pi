...
- 关键协作模块：
  ...
  - `packages/coding-agent/src/task/worktree.ts` — 隔离后端映射(`parseIsolationBackend`)和生命周期(`ensureIsolation`/`cleanupIsolation`)，补丁捕获，分支合并。
...
线协议 schema 由 `task.batch` (默认开启) 切换形状。一个工作单元是任务项 `{ name?, agent?, task, effort?, outputSchema?, schemaMode?, isolated? }`。`isolated` 仅在 `task.isolation.enabled` 为 true **且计划模式已禁用**时存在；`effort` 仅在 `task.enableEffort=true`(默认关闭)时存在。
...
| `isolated` | `boolean` | 否 | 在隔离的工作区中运行并返回补丁。仅当 `task.isolation.enabled` 为 true 且计划模式已禁用时存在；在批次形状中按项设置，在扁平形状中为顶层字段。隔离的代理在完成时被销毁 — 不可唤醒。 |

没有线协议标签字段:TUI/注册表中显示的单行 UI 标签是由 tiny/title 模型从 `task` 文本自动生成的(fire-and-forget)，因此调用者从不提供它。

用户可以在 composer 中用 `^` 标记模型。由此产生的会话本地 `m1`、`m2`、… 伪名可被 task、eval `agent()` 和 `workpool()` 接受为 `agent`；每个都使用绑定到被标记选择器的捆绑 task 模板。持久化、边界和优先级参见 [用户标记的模型代理](../task-agent-discovery.md#用户标记的模型代理)。
...
8. 计划模式换成具有只读工具子集和计划模式提示的 `effectiveAgent`;`runSubprocess(...)` 接收生效的代理。
9. 如果 `isolated`，则需要 git 仓库(`getRepoRoot(...)` / `captureBaseline(...)`)，将 `isolation.backend` 映射到后端种类提示 (`parseIsolationBackend`)，并通过原生 PAL 实现工作区(`ensureIsolation` → `isoResolve`/`isoStart`)，当后端不可用时遍历候选列表。
...
12. `runSubprocess(...)` 创建一个子代理会话，具有隔离的设置快照(继承父设置 — `async.enabled` 和 `bash.autoBackground.enabled` 从父级**继承**，而不是强制禁用；`tier.openai`/`tier.anthropic`/`tier.google` 先通过 `tier.subagent` 重新解析，然后子会话针对其最终模型解析由 task/eval 调度传递的精确 `task.agentServiceTierOverrides[agentName]` 条目并持久化结果；`tools.approvalMode` 强制为 `yolo`，因为无头子代理没有 UI 来确认提示；`advisor.enabled` 强制关闭，除非生成按代理选择加入；每次生成的覆盖可禁用读取摘要并清除隔离运行的额外工作区根)，子代理 `agentId` 等于已分配的 id、子代理内部 URL 路由器/`AgentOutputManager`、输出 schema、系统提示 `CONTEXT` 部分中的共享 `context`（批次调用），以及系统提示中的 IRC 同侪花名册。
...
- 隔离通过 `task.isolation.enabled` 启用；`isolation.backend` 选择 `auto`、`apfs`、`btrfs`、`zfs`、`reflink`、`overlayfs`、`projfs`、`block-clone` 或 `rcopy`，PAL 解析具有回退的实际后端。
- 隔离合并策略： 补丁模式(捕获/应用根补丁)或分支模式(提交到 `omp/task/<id>`,cherry-pick 到父级)。
- 代理源优先级按精确名称先到先得： 项目 `.omp/agents`；用户 `.omp/agent/agents`;CLI → 项目设置 → 用户设置 → 已安装 npm/link 插件顺序中的 OMP 扩展包 `agents/` 根;Claude marketplace 插件代理(项目优先于用户)；然后是捆绑的(`scout`, `reviewer`, `security-reviewer`, `task`, `sonic`)。
