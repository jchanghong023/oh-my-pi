...
| `id` | `string` | 是 | 由 `recall` 返回的记忆 id。 |
| `content` | `string` | 否 | 用于 `update` 的替换记忆文本。 |
| `importance` | `number` | 否 | 用于 `update` 的替换 importance；被限制在 `0..1`。 |
| `replacement_id` | `string` | 否 | 为 `invalidate` 记录的替代记忆 id。 |
...
- `id` 必须直接提供；该工具不按内容搜索。
- Recall 预览默认上限为 500 个字符。在 `update` 之前始终获取 `read memory://<id>`；该 URL 会从调用会话的作用域 bank 中解析完整行。
...
- `read memory://<id>` 在调用会话的作用域 bank 不包含该行时抛出 `Mnemopi memory <id> not found in the calling session's scoped bank`；仅被另一个活动会话持有的行不可达。
