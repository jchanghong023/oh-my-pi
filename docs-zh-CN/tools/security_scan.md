...
- `security.enabled` 默认为 `false`。禁用时，`security_scan` 会从可用工具集中省略，且 `security://` 读取会失败并返回启用提示消息。在 **Settings → Tools → Security** 中启用，或设置 `security.enabled = true`。
...
- 云客户端/导入：`packages/coding-agent/src/security/cloud.ts`
...
| 字段 | 类型 | 用于 | 描述 |
| --- | --- | --- | --- |
| `action` | `"preflight" \| "start" \| "status" \| "cancel" \| "validate" \| "cloud_scans" \| "cloud_start" \| "cloud_status" \| "cloud_pull"` | 所有 | 必需的调度选择器。 |
...
| `credential_id` | 正整数 | 原生 `preflight`；所有云操作 | 固定一个 OAuth 凭据。原生扫描为活动模型提供商选择它；云操作为 `openai-codex` 选择它。 |
...
| `lookback_days` | 正整数或 `"all"` | `cloud_start` | 默认为 `30`;`"all"` 表示发送无限回溯范围。 |
