...
- 关键协作模块：
  ...
  - `packages/tui/src/render/render-utils.ts` — 解析错误去重与显示上限
...
| `path` | `string` | 否 | 一个文件、目录、通配符、有后备文件的内部 URL，或已抓取的网页 URL —— 也可以是其中多项、以分号分隔的列表(`"src; tests"`)。省略或留空时默认为 `.`(工作区根目录)。空条目会被拒绝。内部 URL 的通配符会被拒绝。 |
...
- 若包装器对可见结果进行了截断，文本将以 `Result limit reached; narrow path or increase limit.` 结尾。
...
## 流程
1. `AstGrepTool.execute()` 校验 `pat`，规范化 `skip`，然后将路径解析委托给 `packages/coding-agent/src/tools/path-utils.ts` 中的 `resolveToolSearchScope()`，由它规范化条目、展开分号分隔的列表(外加条件性的逗号/空白拆分)，并拒绝空的 `path` 条目。
...
- 解析问题在 `packages/tui/src/render/render-utils.ts` 中最多渲染 `PARSE_ERRORS_LIMIT = 20` 行；`capParseErrors()` 还将 `details.parseErrors` 上限设为这 20 条唯一条目，`details.parseErrorsTotal` 保存去重后、未截断前的总数。
