...
- 关键协作模块：
  ...
  - `packages/coding-agent/src/utils/github.ts` — `gh` 进程包装器（`github.run/json/text()`），非交互式环境、命令截止时间、有界输出捕获。
  ...
  - `@oh-my-pi/pi-natives/vcs` — git 操作（`vcs.git()` / `vcs.requireGit()`：分支/worktree/配置/push）。
...
## 输出
该工具返回由 `packages/coding-agent/src/tools/gh-common.ts` 中 `buildTextResult()` 构建的单一文本结果。
...
## 流程
1. `GithubTool.createIf()` 仅在 `github.available()` 在 `PATH` 上找到 `gh` 时才暴露该工具。
...
4. CLI 执行通过 `packages/coding-agent/src/utils/github.ts` 中的 `github.run/json/text()`：
   - 在非交互式环境下使用 `Bun.spawn()` 启动 `gh ...`，带有 5 分钟截止时间（`GH_COMMAND_TIMEOUT_MS`）和 8 MiB 捕获输出上限；
...
7. `pr_checkout` 首先解析 PR 元数据，然后在执行任何 git 修改前进入 `withRepoLock()`（`packages/coding-agent/src/utils/repo-lock.ts`），以避免对同一主仓库的并行 checkout 调用在共享的 `.git` 状态上产生竞争。
8. `pr_push` 从 git 分支配置中读取 PR head 元数据，推导 refspec，使用 `repository.push()`（`@oh-my-pi/pi-natives/vcs`）推送，然后通过 `invalidateAllForNumber()` 使所推送 PR 的 `pr://` 缓存行失效，以便下一次 `pr://` 读取反映该推送。
...
| 批处理 | 是。`pr` 可为 `string[]`；每个 PR 并行解析，但 git 修改由 `withRepoLock()` 按主仓库串行化。 |
...
- 已存在 worktree 的检测通过来自 `repository.worktrees()`（`@oh-my-pi/pi-natives/vcs`）的分支 ref `refs/heads/pr-<number>` 进行。
- 新建 worktree 在验证路径既未被注册也不存在于磁盘上后，调用 `repository.worktreeAdd(finalWorktreePath, localBranch, false, signal)`。
...
- 如果 `refs/heads/pr-<number>` 已存在但指向不同的 commit,checkout 会失败，除非 `force=true`，此时 `repository.createBranch(..., force=true)` 将其重置为已 fetch 的 PR head。
...
- 失败 job 的日志通过 `gh api /repos/<repo>/actions/jobs/<jobId>/logs` 经由 `github.run()` 获取，而不是 `json()`。非零退出码会留下 `available: false`，而不是让整个 watch 失败。
...
- 子进程 / 原生绑定
  ...
  - `pr_checkout` 和 `pr_push` 还会通过 `@oh-my-pi/pi-natives/vcs`（`vcs.requireGit()`）调用 git 操作，并由来自 `packages/coding-agent/src/utils/repo-lock.ts` 的 `withRepoLock()` 串行化。
...
- 后台工作 / 取消
  ...
  - `GithubTool.execute()` 被包装在 `untilAborted()` 中；`github.run()` 将 abort signal 转发到 `Bun.spawn()`。

## 限制与上限
- 搜索结果默认值：`10`（`packages/coding-agent/src/tools/gh-search.ts` 中的 `SEARCH_LIMIT_DEFAULT`）。
- 搜索结果最大值：`50`(`SEARCH_LIMIT_MAX`)。
- `pr://` 视图中的 PR 文件预览：仅前 `50` 个文件(`gh-search.ts` 中的 `FILE_PREVIEW_LIMIT`)。对于达到 GitHub 20,000 行限制而拒绝的聚合 diff,`pr://<N>/diff` 获取器会回退到分页 files API(每页 `100` 个文件，最多 `3000` 个文件)；二进制或单个体积过大的 patch 仍会列出，并带有 unavailable-patch 标记。
...
## 错误
- 当 `gh` 未安装时，工具创建被完全跳过。
- 如果在执行时 `gh` 缺失，`github.run()` 抛出 `ToolError("GitHub CLI (gh) is not installed...")`。
- `github.text()/json()` 将常见错误映射为面向模型的消息：
