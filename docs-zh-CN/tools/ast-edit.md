...
- 面向模型的提示词：`packages/coding-agent/src/prompts/tools/ast-edit.md`
- 关键协作者：
...
共享的 AST 模式语法与语言目录：参见 [`ast_grep`](./ast-grep.md#输入)。
...
- 工具总是先进行预览（直接结果中为 `applied: false`）。实际的文件写入仅在之后通过一次向 `xd://resolve` 发送的纯文本 `write` 完成；正文即为原因。
...
- 解析问题在 `packages/tui/src/render/render-utils.ts` 中通过 `capParseErrors(...)` 去重并封顶为 `PARSE_ERRORS_LIMIT = 20` 条；`details.parseErrors` 携带封顶后的列表，`details.parseErrorsTotal` 携带封顶前的去重计数。
