...
SDK 生成的追加内容（用于已启用的记忆/自动学习功能以及 MCP 指引）会在用户提供追加文本之前进行合并。这些生成的块可能以 `## MCP Server Instructions` 结尾，其文本声明自身由服务端控制且未经校验。只要生成的块位于用户提供的文本之前，该文本就会在其自身的 `## User Instructions` 标题下渲染，从而不会被误读为服务端所属章节的尾随段落。单独使用时——既无生成的块、也没有 `SYSTEM.md`——追加文本会原样发出，不带标题。
