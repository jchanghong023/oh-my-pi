...
Agent Hub 是用于查看和控制当前会话关联子代理的交互式 TUI。它集成了实时代理列表、各代理的活动与用量、转写记录访问、引导、恢复（revive）和终止（kill）控制。主代理不在列表中显示，因为它的对话就是当前会话视图本身。
...
| 双击 `←`       | 在主会话编辑器为空且当前会话有代理可显示时，打开 Hub。  |

运行 `/hotkeys` 查看当前生效的组合键。可在 `~/.omp/agent/keybindings.yml` 中重新映射任一操作：
...
表头汇总所有已计量代理的状态和用量。按 `t` 可在稳定的扁平列表与父子树视图之间切换。
...
只有 `parked` 状态的代理可以恢复。`x` 立即生效；仅在确实打算丢弃该代理实例时使用。
...
子代理运行期间，编辑器上方固定的 `Subagents` 块会列出所有活跃代理——无论同步任务调用，还是脱离的后台派生代理。

列表始终保持简短：默认（`display.pinnedAgents: collapsed`）只显示几行外加一个展开器，也可以列出全部（`full`）或完全隐藏（`off`）。在 `tui.mouse` 开启时，点击展开器可在「显示几行」与「列出全部」两种状态之间切换。

启用 `tui.mouse` 后，无需先打开 Hub，即可直接在主会话中点击活跃的子代理卡片和跳转列表行。点击会聚焦该卡片最近的代理（跳转列表行则聚焦其对应的确切代理）；聚焦已停放的代理会将其恢复。悬停在活跃目标上会先将其高亮，让你能预知点击将打开的对象。

只有当前位于实时视口内的行才可点击——已退场的转写行位于终端回滚缓冲区中，点击无法映射回对应内容。启用捕获（capture）期间，终端手势会发生改变：文本选择变为 Shift+拖动，滚轮滚动变为 Shift+滚轮。该功能默认关闭。
...
Agent Hub 是面向用户的实时会话视图。相邻的命令和内部 URL 承担更专门的用途：
