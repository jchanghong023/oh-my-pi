...
- 无冲突时只做 Git staged 检查，不运行 Bun、测试或全仓扫描；有冲突时只验证冲突解决涉及的文件和 workspace。
...
- annotated tag 时仅允许继续读取该返回对象自身的 `object.url`，直到得到一个 commit；最多追加一次解引用，仍不是 commit 则 STOP。
- 本地分支：`main`、`upstream`
...
“上游同步记录”必须唯一且只记录 `baseline_tag`。结构缺失或歧义 → STOP；不要进入扩散式调查。
...
只做本地 SemVer 数值比较：候选必须严格高于基线。通过后进入第 1 节；不通过立即 STOP。不得查询旧 Release 页面或其他 tag 辅助判断。
...
完整读取三个治理文件。确认 `origin` 的 fetch/push URL 唯一解析为 `jchanghong023/oh-my-pi`；只用于精确 SHA 历史补全和维护 `origin/upstream`，不得检查任何其他远程分支。
...
若 `release_commit` 已是本地 `main` 的祖先，标记 `already_contained=true`，跳到第 6 节对账。
...
1. 冲突解决涉及 TypeScript 时，审计后运行一次 `bun run fastcheck`。
...
- 若 `release_tag != baseline_tag` 但 `release_commit == baseline_release_commit`，说明新 Release tag 指向相同代码：保留原同步日期和 Merge，只更新版本及唯一记录，不下载历史、不运行测试。
...
- 确认 `origin` 的唯一 push URL 解析为 `jchanghong023/oh-my-pi`。
- 不查询任何其他远程分支。
...
```bash
git_guarded ls-remote --heads origin refs/heads/upstream
```

查询失败 → STOP；空输出表示不存在；多于一条或格式异常 → STOP。

- 不存在：用空 expected-value lease 创建。
- 已存在且 SHA 等于 `release_commit`：不 push。
- 已存在但不同：只有在第 0.4-A（基线 tag+commit 与最新完全一致）或新 Release 已完成本地 merge/docs 时，才可使用精确旧 SHA 的 `--force-with-lease`。
...
push 后或 push 结果不确定时，只重新查询一次实际 ref；达到目标则成功，否则 STOP，不刷新 lease 重试。
