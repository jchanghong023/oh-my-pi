...
- 镜像只在本地 `main` 合入、验证和 fork 快照完成后更新；正常演进普通 push，非 fast-forward 漂移只允许按 skill 对 `origin/upstream` 使用精确旧 SHA 的 `--force-with-lease`。
...
- 所有 Git 命令 MUST 隔离本地 hooks、rerere、commit/push signing、自动 maintenance/GC、隐式 tag/submodule push 和交互认证；未知自定义 merge/filter driver 必须停止。
...
- NEVER 在上游同步期间运行会编译、检查、测试、lint、fmt、clippy、生成或打包 Rust 源码/native 二进制的命令；NEVER 运行根级 `bun run check`、`cargo`、`bazel`、`nix build`、Docker/native build 或间接触发这些工作的脚本。
- TS/JS 对 native 包装层的纯静态检查 MAY 运行，但 MUST 先确认脚本不调用 Rust/native 工具链。
