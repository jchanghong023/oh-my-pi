...
| `fullsend`    | Makes speed and verified quality joint top priorities without monetary or token constraints. It chooses direct work or parallel delegation by shortest expected completion time at equal quality; equal-time paths delegate because clean subagent context is the quality tie-breaker. Required verification remains mandatory. |
...
The slash commands `/ultrathink`, `/orchestrate`, `/workflowz`, and `/fullsend` accept optional task text. For example, `/fullsend complete and verify the release` sends `fullsend complete and verify the release`; invoking one without task text sends only its keyword.

## Matching rules

Matching is deliberate so source code and paths do not accidentally change agent behavior:

- Use the exact lowercase spelling. `Ultrathink`, `Orchestrate`, `Workflowz`, and `Fullsend` do not trigger.
